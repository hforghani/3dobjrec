function key_points = sift_scale(im)
    im_s = size(im);
	octaves = floor(log2(min(im_s(1:2))));
	intervals = 10;
    ...
    for i = 1:octaves
        for j = 1:intervals+3
            subplot(octaves, intervals+3, (i-1)*(intervals+3)+j);
            imshow(m_gList{i,j});
        end
    end
    figure(2);
    for i = 1:octaves
        for j = 1:intervals+2
            subplot(octaves, intervals+2, (i-1)*(intervals+2)+j);
            imshow(m_dogList{i,j});
        end
    end
...
%     temp = upsample(imgGray, 2);
%     temp = upsample(temp', 2)';
%     h = fspecial('gaussian', 5);
%     m_gList{1,1} = imfilter(temp, h, 'same');
	...
	for i = 1 : m_numOctaves
		...
		if i < m_numOctaves
			% Resample the image
			temp = downsample(m_gList{i,1}, 2);
            m_gList{i+1,1} = downsample(temp', 2)';
		...
		for j = 2 : m_numIntervals + 1
			...
			for xi = 2 : size(m_dogList{i,j}, 2) - 1
                for yi = 2 : size(m_dogList{i,j}, 1) - 1
                    ...
                    middle_border = middle(yi-1:yi+1, xi-1:xi+1);
                    middle_border(5) = [];

					% Check for a maximum or minimum
					if (all(currentPixel < middle_border) && ...
                            all(all(currentPixel < up(yi-1:yi+1, xi-1:xi+1))) && ...
                            all(all(currentPixel < down(yi-1:yi+1, xi-1:xi+1)))) ...
                            || ...
                            (all(currentPixel > middle_border) && ...
                            all(all(currentPixel > up(yi-1:yi+1, xi-1:xi+1))) && ...
                            all(all(currentPixel > down(yi-1:yi+1, xi-1:xi+1))))
					...
					% The contrast check
					if justSet && abs(middle(yi, xi)) < CONTRAST_THRESHOLD
